...
/**
 * GlassToolbar - A floating toolbar with a prominent add button
 * Uses Expo UI's SwiftUI components for native liquid glass effect
 */
export function GlassToolbar({ bottomOffset = 100 }: GlassToolbarProps) {
  const insets = useSafeAreaInsets();
  const { triggerAddEventFlow } = useAddEventFlow();

  // Only render on iOS - Android doesn't support the glass effect
  if (Platform.OS !== "ios") {
    return null;
  }
