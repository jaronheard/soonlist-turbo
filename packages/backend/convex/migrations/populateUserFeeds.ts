    ...
    for (let i = 0; i < events.length; i += batchSize) {
      ...
      for (const event of batch) {
        ...
        if (processedCount % 100 === 0) {
          console.log(`Processed ${processedCount}/${events.length} events, added ${addedCount} feed entries`);
        }
      }
    }
    
    console.log(`Migration complete! Processed ${processedCount} events, added ${addedCount} feed entries`);
...
export const cleanupOrphanedFeedEntries = internalMutation({
  handler: async (ctx) => {
    ...
    console.log(`Cleanup complete! Deleted ${deletedCount} orphaned feed entries`);
